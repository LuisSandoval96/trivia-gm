...
console.log('🚀 Starting Trivia GM Server...');
console.log('📊 PORT:', PORT);
console.log('🔧 NODE_ENV:', process.env.NODE_ENV);
console.log('🔐 ADMIN_PASSWORD configured:', ADMIN_PASSWORD ? 'Yes' : 'No');
...
io.on('connection', (socket) => {
    ...
    socket.on('join-admin', (data) => {
        console.log('👨‍💼 Admin join attempt:', data);
        socket.join('admin');
...
function startQuestion() {
    console.log(`❓ Starting question ${gameState.currentQuestion + 1}/${questions.length}`);
...
function showResults() {
    console.log('📊 Showing results for question', gameState.currentQuestion + 1);
    const currentQ = questions[gameState.currentQuestion];
...
// Iniciar servidor con la estabilidad probada
const serverInstance = server.listen(PORT, '0.0.0.0', (error) => {
    if (error) {
